...
import { deletePago } from '$lib/modules/cuentas-pagar/services/cuentasPagar.service';
import { PK_COLUMN } from '$lib/modules/cuentas-pagar/config/pago.config';
...
const handler: RequestHandler = async ({ request, cookies }) => {
	...
	if (!id) return json({ success: false, message: 'ID de pago inválido' }, { status: 400 });

	const result = await deletePago(supabase, id);
