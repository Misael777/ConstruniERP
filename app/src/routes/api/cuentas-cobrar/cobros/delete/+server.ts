...
import { deleteCobro } from '$lib/modules/cuentas-cobrar/services/cuentasCobrar.service';
import { PK_COLUMN } from '$lib/modules/cuentas-cobrar/config/cobro.config';
...
const handler: RequestHandler = async ({ request, cookies }) => {
	...
	if (!id) return json({ success: false, message: 'ID de cobro inválido' }, { status: 400 });

	const result = await deleteCobro(supabase, id);
