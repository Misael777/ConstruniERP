		...
		<div class="bg-white rounded-3xl shadow-2xl border border-slate-100 max-w-4xl w-full overflow-hidden flex flex-col max-h-[90vh] scale-100 transition-transform duration-300">
			...
			<div class="bg-slate-50 border-b border-slate-100 px-6 py-4 flex items-center justify-between">
				<div class="flex items-center gap-2">
					...
					<h3 class="font-bold text-slate-800 text-base">{editingId ? 'Editar Registro de Consultoría' : 'Registrar Nueva Consultoría'}</h3>
				</div>
			...
			<form onsubmit={(e) => { e.preventDefault(); guardar(); }} class="flex-1 overflow-y-auto p-6 space-y-6">
				...
				<div>
					...
					<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
						<div>
							<label class="block text-xs font-semibold text-slate-600 mb-1">Nombre del Servicio *</label>
							<input 
								type="text" 
								bind:value={formNombre} 
								placeholder="Ej. Estudio Hidrogeológico del Valle de Asia" 
								required 
						...
						<div>
							...
							<input 
								...
								placeholder="Ej. CON-010" 
								class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 focus:bg-white outline-none transition-all text-xs" 
							/>
						</div>
						<div>
							<label class="block text-xs font-semibold text-slate-600 mb-1">Área Responsable</label>
							<select 
				...
				<div>
					<h4 class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-3 border-b border-slate-100 pb-1">Finanzas e Inventario</h4>
					<div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
						<div>
							<label class="block text-xs font-semibold text-slate-600 mb-1">Precio de Venta (S/)</label>
							<input 
								type="number" 
								step="0.01"
						...
						<div>
							<label class="block text-xs font-semibold text-slate-600 mb-1">Existencia / Cantidad</label>
							<input 
								type="number" 
								step="0.1"
						...
						<div>
							<label class="block text-xs font-semibold text-slate-600 mb-1">Valor Mínimo (Alerta stock)</label>
							<input 
								type="number" 
								step="0.1"
						...
						<div>
							<label class="block text-xs font-semibold text-slate-600 mb-1">Valor Máximo (Límite stock)</label>
							<input 
								type="number" 
								step="0.1"
						...
						<div>
							<label class="block text-xs font-semibold text-slate-600 mb-1">Link a Proforma o Expediente</label>
							<input 
								type="text" 
				...
				<div>
					...
					<div class="grid grid-cols-1 md:grid-cols-3 gap-4">
						<div>
							<label class="block text-xs font-semibold text-slate-600 mb-1">Especialidad (Caract. 1)</label>
							<input 
								type="text" 
								bind:value={formCaract1} 
								placeholder="Ej. Estructuras, Geotecnia" 
								class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 focus:bg-white outline-none transition-all text-xs" 
							/>
						</div>
						<div>
							<label class="block text-xs font-semibold text-slate-600 mb-1">Entregable Principal (Caract. 2)</label>
							<input 
								type="text" 
								bind:value={formCaract2} 
								placeholder="Ej. Informe Técnico, Planos CAD" 
								class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 focus:bg-white outline-none transition-all text-xs" 
							/>
						</div>
						<div>
							<label class="block text-xs font-semibold text-slate-600 mb-1">Fase / Tipo (Caract. 3)</label>
							<input 
								type="text" 
								bind:value={formCaract3} 
								placeholder="Ej. Pre-factibilidad, Definitivo" 
								class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 focus:bg-white outline-none transition-all text-xs" 
							/>
						</div>
					</div>
				...
				<div>
					...
					<div>
						<label class="block text-xs font-semibold text-slate-600 mb-1">Descripción del Servicio</label>
						<textarea 
							bind:value={formDescripcion} 
							placeholder="Escribe el alcance, cronograma propuesto u observaciones de esta consultoría..." 
							rows="3" 
							class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 focus:bg-white outline-none transition-all text-xs"
						></textarea>
					</div>
				...
				<div class="pt-4 border-t border-slate-100 flex justify-end gap-3">
					<button 
						...
						class="px-4 py-2 bg-slate-100 text-slate-700 rounded-xl hover:bg-slate-200 text-xs font-semibold transition-colors cursor-pointer animate-fade-in"
					>
						Cancelar
					</button>
					<button 
